...
import { Container, Form, Segment, Header, Table, Icon, Button, Message } from "semantic-ui-react";
...
<Container style={{ marginTop: "7em" }}>
        <Header>
          {
            this.state.orderStatus == 0 ? "Bestelling status: Niet geaccepteerd" : "Bestelling status: Geaccepteerd"
          }
        </Header>
        ...
        >
        {
            this.state.updateFormError ? <Message size='large' error 
        ...
        {
            this.state.orderUpdated ? <Message size='large' success 
              ...
              return (
                <Table.Row key= {index}>
                  ...
                  <Table.Cell textAlign="right">
                    <Button disabled = {this.state.orderStatus == 1} onClick={this.orderProduct.bind(this, product.id, product.quantity, product)}>
                        Bestel product
                    </Button>
                  </Table.Cell>
                </Table.Row>
  ...
  orderProduct = (productId, quantity, product) => {
    ...
    .then(data => {
      ...
      this.setState({...this.state, serverUpdateOrderResponse: data.response, updateFormError: data.isError, orderUpdated: data.orderUpdated, formIsLoading: false})
    });
  }
  ...
  updateOrder = () => {
    ...
    .then(data => {
      console.log(data);
      this.setState({...this.state, orderStatus: data.orderStatus,  serverUpdateOrderResponse: data.response, updateFormError: data.isError, orderUpdated: data.orderUpdated, formIsLoading: false})
    });
  };
